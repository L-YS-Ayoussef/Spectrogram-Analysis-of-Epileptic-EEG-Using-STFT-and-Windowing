# 🧠 Spectrogram Analysis of Epileptic EEG Using STFT and Windowing
...
- **Objective 1 – Spectrogram & STFT Implementation**
  - Understanding spectrograms conceptually and programmatically.
  - Implementing STFT from scratch using the FFT (`fft`) function.
  - Converting multi-channel EEG signals into time–frequency representations.

- **Objective 2 – Time–Frequency Resolution & Window Study**
...
#### 1️⃣ Importing and Organizing EEG Records
...
- `ReadEDF.m`, `EdfToMat.m`, `SaveEDF.m` – Reading and converting EDF data to MATLAB `.mat` format.
- `reading_Data.m` – High-level script for loading EEG data and headers.
- `chb12_29_data.mat`, `chb12_29_header.mat`, `chb12_32_data.mat`, `chb12_32_header.mat`, `chb12-summary.txt` – Raw and annotation files.
- `ictal_data.mat`, `seizure_free_data.mat` – Processed datasets containing ictal and seizure-free segments.

---

#### 2️⃣ Channel Selection and Averaging
...
- `user.m` – User interface / control logic (channel selection and options).
- `reading_Data.m` – Provides the loaded multi-channel EEG data.
- `ictal_averaged.mat`, `seizure_free_averaged.mat` – Averaged single-channel ictal and seizure-free signals.
...
- Shorter windows → **better time resolution**, **poorer frequency resolution**.
- Longer windows → **better frequency resolution**, **poorer time resolution**.

**Main files involved:**

- `compute_spectrogram.m` – Core STFT + spectrogram computation.
- `spectrography.m` – Script for experimenting with different resolutions.
- `spectrogram_analysis.m` – Visualization and comparison of different (time, frequency) settings.

---

#### 4️⃣ Overlapping Ratio
...
- Higher overlap → smoother spectrogram and less time aliasing, at the cost of more computation.
- No or low overlap → faster but may look more “blocky” in time.

**Main files involved:**

- `compute_spectrogram.m` – Implements overlapping logic.
- `spectrography.m`, `windowing_analysis.m` – Experiments and plots for different overlap settings.
...
- `windowing_analysis.m` – Focused analysis of different window sizes.
- `spectrogram_analysis.m` – Displays spectrograms for varying window sizes.

---

#### 6️⃣ Window Types and Parameters
...
- `compute_spectrogram.m` – Applies the selected window function.
- `windowing_analysis.m` – Compares Rectangular, Triangular, Hamming, and Blackman windows.
- `spectrography.m`, `spectrogram_analysis.m` – Visual inspection of the spectrograms under different windows.

---

### III. Plotting & Visualization

#### 7️⃣ Spectrogram Display
...
- `DSP_Assignment.mlx` – **Live Script notebook** that documents and runs **all project tasks** end-to-end, combining:
  - Data loading
  - Pre-processing
...
- `user.m` – User interface / control script (channel selection and parameter input).
- `EdfToMat.m`, `ReadEDF.m`, `SaveEDF.m` – EDF ↔ MAT conversion utilities.
- `ictal_data.mat`, `seizure_free_data.mat` – Extracted ictal and seizure-free records.
- `ictal_averaged.mat`, `seizure_free_averaged.mat` – Channel-averaged versions of the above.
- `spectrogram_analysis.m`, `spectrography.m`, `windowing_analysis.m` – Analysis and visualization scripts for different experimental setups.
